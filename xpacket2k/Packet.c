...
)
{
	...
	for(i = 0; i < BufferCount; i++)
	{
		...
		PrintBlock((PVOID)VirtualAddress, Length);
		NdisGetNextBuffer(Buffer, &Buffer);
	}

	NdisQueryBuffer(FirstBuffer, &VirtualAddress, &Length);
	if(PrintEthernetFrame((PETHERNET_FRAME)VirtualAddress
		, Length) == ETHERNET_FRAME_TYPE_TCPIP)
	{
		NdisGetNextBuffer(FirstBuffer, &FirstBuffer);
		if(FirstBuffer != NULL)
			PrintTcpIpHeader(FirstBuffer);
	}
...
)
{
	...
	if(Buffer == NULL || Length < IP_HEADER_LENGTH)
	{
	...
	HeaderLength = PrintIp(pIpHeader, Length);
	...
	case PROTOCOL_TCP:
		dprintf(("    Protocol: %s(0x%02X)\n", "TCP", pIpHeader->Protocol));
		{
			...
			if((Length - HeaderLength) < TCP_HEADER_LENGTH)
			{
				NdisGetNextBuffer(Buffer, &Buffer);
				NdisQueryBuffer(Buffer, &VirtualAddress, &Length);
				if(Buffer != NULL && Length >= TCP_HEADER_LENGTH)
				{
			...
			{
				pTcpHeader = (PTCP_HEADER)((DWORD)VirtualAddress + HeaderLength);
			}
			   ...
			   || ntohs(pTcpHeader->DestinationPort) == 137)
			{
				NdisGetNextBuffer(Buffer, &Buffer);
				NdisQueryBuffer(Buffer, &VirtualAddress, &Length);
				if(Buffer != NULL && Length >= NETBIOS_MIN_PACKET_SIZE)
					MakeNameList((char*)VirtualAddress);
			}
		}
	...
	case PROTOCOL_UDP:
		dprintf(("    Protocol: %s(0x%02X)\n", "UDP", pIpHeader->Protocol));
		{
			...
			if((Length - HeaderLength) < UDP_HEADER_LENGTH)
			{
				NdisGetNextBuffer(Buffer, &Buffer);
				NdisQueryBuffer(Buffer, &VirtualAddress, &Length);
				if(Buffer != NULL && Length >= UDP_HEADER_LENGTH)
				{
			...
			{
				pUdpHeader = (PUDP_HEADER)((DWORD)VirtualAddress + HeaderLength);
			}
	...
	case PROTOCOL_ICMP:
		dprintf(("    Protocol: %s(0x%02X)\n", "ICMP", pIpHeader->Protocol));
		{
			...
			if((Length - HeaderLength) < ICMP_HEADER_LENGTH)
			{
				NdisGetNextBuffer(Buffer, &Buffer);
				NdisQueryBuffer(Buffer, &VirtualAddress, &Length);
				if(Buffer != NULL && Length >= ICMP_HEADER_LENGTH)
				{
			...
			{
				pIcmpHeader = (PICMP_HEADER)((DWORD)VirtualAddress + HeaderLength);
			}
